...
namespace Trajectory.Views
{
    public class CExTabControl : TabControl
    {
        ...
        public CExTabControl()
        {
            ...
            // 设置默认颜色
            m_itemSelectedColor = Color.White;
            m_itemUnSelectedColor = Color.Silver;
        ...
        {
            foreach (var item in m_listPage)
            {
        ...
        public void RemovePage(ITabPage page)
        {
            ...
            foreach (var item in m_listPage)
            {
                if (page.Title.Trim() == item.Title.Trim())
                {
                    removeindex = m_listPage.IndexOf(item);
                    break;
                }
            }
            this.TabPages.RemoveAt(removeindex);
            m_listPage.RemoveAt(removeindex);
            // 更新其它页面的索引吧？
